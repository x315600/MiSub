...
const excludeRuleLines = computed(() => {
  const text = editingSubscription.value?.exclude ?? '';
...
const handleSaveSubscriptionWithValidation = () => {
...
<template>
  ...
  <Modal v-if="editingNode" v-model:show="showNodeModal" @confirm="handleSaveNode" @vue:mounted="console.log('NodeModal Mounted, data:', editingNode)">
    ...
    <Modal v-if="editingSubscription" v-model:show="showSubModal" @confirm="handleSaveSubscriptionWithValidation" @vue:mounted="console.log('SubModal Mounted, data:', editingSubscription)">
